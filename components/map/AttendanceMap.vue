<template>
  <div class="attendance-map-wrapper">
    ...
    <div class="attendance-legend">
      <div class="legend-title">Attendance</div>
      <div v-for="item in LEGEND" :key="item.key" class="legend-row">
        <span class="legend-dot" :style="{ background: item.color }" />
        <span class="legend-label">{{ item.label }}</span>
      </div>
      <div class="legend-divider" />
      <label v-if="lsoaLoaded" class="legend-toggle-row" @click.prevent="toggleLSOALayer">
        <span class="legend-label">LSOA boundaries</span>
        <span class="legend-toggle" :class="{ 'legend-toggle--on': lsoaVisible }">
          <span class="legend-toggle-thumb" />
        </span>
      </label>
      <div class="legend-divider" />
      <div class="legend-hint">Markers show all pupils per postcode.<br />Click to expand individual pupils.</div>
    </div>
...
function generatePupils(record) {
  ...
  return catAssignments.map((cat, i) => {
...
const props = defineProps({
  /** URL to the geocoded JSON produced by scripts/geocode-attendance.js */
  dataUrl: {
    type: String,
    default: '/data/southampton-attendance.json',
  },
  center: {
    type: Array,
...
/** postcode → { marker: maplibregl.Marker, el: HTMLElement, record } */
const collapsedMarkers = new Map()
/** Active exploded dot markers */
...
const LSOA_SOURCE = 'am-lsoa-source'
const LSOA_FILL   = 'am-lsoa-fill'
const LSOA_LINE   = 'am-lsoa-line'

async function loadLSOALayer() {
  try {
    const res = await fetch('/geo/southampton-lsoa.geojson')
    if (!res.ok) throw new Error(`HTTP ${res.status}`)
    const geojson = await res.json()

    map.value.addSource(LSOA_SOURCE, { type: 'geojson', data: geojson })
    ...
    console.warn('Could not load LSOA layer:', err.message)
  }
}
...
function toggleLSOALayer() {
  ...
  const vis = lsoaVisible.value ? 'visible' : 'none'
  map.value.setLayoutProperty(LSOA_FILL, 'visibility', vis)
  map.value.setLayoutProperty(LSOA_LINE, 'visibility', vis)
}
...
/** Marker diameter in px, sqrt-scaled by pupil count */
function markerSize(total) {
  return Math.round(Math.min(56, Math.max(20, 7 + Math.sqrt(total) * 5.2)))
}

/** Build an SVG donut-pie for the collapsed marker */
function pieSvg(record, size) {
  const r   = size / 2 - 1
  const cx  = size / 2
  const cy  = size / 2
  const ir  = r * 0.38         // inner radius (donut hole)
...
/** Create the DOM element for a collapsed postcode marker */
function makeCollapsedEl(record) {
  const size = markerSize(record.total)
  const el = document.createElement('div')
  el.className = 'am-collapsed'
  // MapLibre sets transform on `el` for geo-positioning — never touch it.
  // All visual effects go on this inner wrapper instead.
...
/**
 * Returns [dx, dy] pixel offsets for n points arranged in
 * concentric rings (innermost first).
 */
function spiralOffsets(n) {
  ...
  while (offsets.length < n) {
    ...
    // Stagger alternate rings by half a step for tighter packing
...
function collapseExploded() {
  ...
  if (explodedPostcode) {
    const entry = collapsedMarkers.get(explodedPostcode)
    if (entry) {
...
function explodePostcode(record) {
  ...
  // Slightly dim the centre marker so the individual dots read clearly,
  // but keep it fully visible so users can click it to collapse.
  const entry = collapsedMarkers.get(record.postcode)
  if (entry) {
    const inner = entry.el.querySelector('.am-collapsed-inner')
    if (inner) inner.style.opacity = '0.55'
  }

  // Build flat list of pupils ordered SPA → PA → Below → Above
  const pupils = []
  for (const cat of ORDERED_CATS) {
    const count = record[cat.toLowerCase()]
...
async function loadData() {
  ...
  try {
    ...
    if (!res.ok) throw new Error(`HTTP ${res.status} loading ${props.dataUrl}`)
  ...
  for (const record of records) {
    ...
    collapsedMarkers.set(record.postcode, { marker, el, record })
  }

  // Clicking the map background collapses any open explode and closes drawer
  map.value.on('click', () => {
    collapseExploded()
    closeDrawer()
  })
...
onBeforeUnmount(() => {
  collapseExploded()
  for (const { marker } of collapsedMarkers.values()) marker.remove()
  collapsedMarkers.clear()
  map.value?.remove()
})
</script>
...
.attendance-legend {
  ...
  min-width: 210px;
  pointer-events: none; /* individual interactive rows override this */
}
...
<!-- Global styles for marker elements (not scoped — applied to dynamic DOM) -->
<style>
/* MapLibre sets `transform` on .am-collapsed for geo-positioning.
   Never add transform or transition here — it will override MapLibre's placement. */
.am-collapsed {
  cursor: pointer;
}

/* All visual effects go on the inner wrapper that MapLibre doesn't touch. */
.am-collapsed-inner {
