...
export const SouthamptonWithClusters = {
  render: () => ({
    components: { ClusterMap, RangeSlider, Drawer },
    setup() {
      const isKeyDrawerOpen = ref(false)
      const isDetailDrawerOpen = ref(false)
      const selectedLSOA = ref(null)
      const threshold = ref(65)
      const lsoaFillOpacity = computed(() => (threshold.value / 100) * 0.6 * 0.25)

      function onLSOASelect(feature) {
        selectedLSOA.value = feature
        isDetailDrawerOpen.value = true
      }

      function onLSOADeselect() {
        isDetailDrawerOpen.value = false
        selectedLSOA.value = null
      }
    ...
    template: `
      <div style="position:relative; width:100vw; height:100vh; overflow:hidden; font-family:'Open Sans',sans-serif;">
        <ClusterMap
          city="southampton"
          :showLSOA="true"
          lsoaFillColor="#1a7fa0"
          lsoaLineColor="#125f7a"
          :lsoaFillOpacity="lsoaFillOpacity"
          :showDistrictBoundaries="false"
          :attendanceThreshold="threshold"
          :enableLSOAInteraction="true"
          @lsoaSelect="onLSOASelect"
          @lsoaDeselect="onLSOADeselect"
        />
        ...
        >
          ...
          <div style="padding:20px 20px 28px;">
            ...
            <RangeSlider
              v-model="threshold"
              label="Filter by Minimum Attendance"
              :min="0"
              :max="100"
              unit="%"
              :showGradient="true"
        ...
        >
          <template #header>
            <div style="padding:20px 20px 0; display:flex; justify-content:space-between; align-items:flex-start;">
              <div>
                <h2 style="font-size:18px; font-weight:700; color:#1a7fa0; margin:0 0 4px 0;">Area Detail</h2>
                <p style="font-size:12px; color:#888; margin:0;">Selected LSOA information</p>
              </div>
              <button
                @click="isDetailDrawerOpen = false"
          ...
          <div style="padding:20px;">
            <template v-if="selectedLSOA">
              <div style="background:#e8f4f8; border:1px solid #b3d9e6; border-radius:8px; padding:16px;">
                <div style="font-size:11px; font-weight:700; text-transform:uppercase; letter-spacing:0.08em; color:#1a7fa0; margin-bottom:6px;">
                  {{ selectedLSOA.LSOA21CD }}
                </div>
                <div style="font-size:16px; font-weight:700; color:#1c2b3a; margin-bottom:10px; line-height:1.3;">
                  {{ selectedLSOA.LSOA21NM }}
